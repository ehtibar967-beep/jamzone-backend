'use strict';
// JamZone Backend Lite: health + realtime WebSocket чат для твоего HTML-приложения.
// Работает на Render.com без зависимостей.
...
const history = []; // {from,to,text,ts,...} хранится пока сервер не перезапустится
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/') {
    ...
    return res.end('<h1>🟢 JamZone Backend работает</h1><p>Health: <a href="/health">/health</a></p><p>WebSocket: /ws</p>');
  }
...
function handleMsg(conn, msg) {
  ...
  if (msg.type === 'typing' && msg.to) {
    sendToName(cleanName(msg.to), { type: 'typing', from: conn.name, fromName: conn.displayName });
    return;
  }
  if (msg.type === 'dm' && msg.to) {
    ...
    console.log('📩', conn.name, '->', to, item.text.slice(0, 80));
    return;
  }
...
function cleanName(x) { return String(x || '').replace('@','').trim().toLowerCase().replace(/[^a-z0-9_\-.а-яё]/gi, '').slice(0, 40); }
function sendToName(name, obj) { const set = onlineByName.get(name); if (set) for (const c of set) c.send(obj); }
...
server.listen(PORT, HOST, () => {
  console.log('✅ JamZone Backend запущен');
